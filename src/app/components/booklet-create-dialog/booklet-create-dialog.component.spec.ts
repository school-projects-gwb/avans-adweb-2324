...
import {
  ...
  BookletDialogData,
} from './booklet-create-dialog.component';
...
describe('BookletCreateDialogComponent', () => {
  ...
  it('should initialize form with provided data', () => {
    expect(component.basicInfoFormGroup.controls['name'].value).toBe(
      'Test Booklet'
    );
    expect(component.basicInfoFormGroup.controls['description'].value).toBe(
      'Test Description'
    );
  });
  ...
  it('should add user email to booklet', () => {
    const mockBooklet = {
      addAuthenticatedUserEmail: jasmine.createSpy('addAuthenticatedUserEmail'),
    };
    component.data.booklet = mockBooklet;
    component.addAuthenticatedUserEmail('test@example.com');
    expect(mockBooklet.addAuthenticatedUserEmail).toHaveBeenCalledWith(
      'test@example.com'
    );
    expect(
      component.userAccessFormGroup.controls['userEmailToModifyAccess'].value
    ).toBeNull();
  });

  it('should remove user email from booklet', () => {
    const mockBooklet = {
      removeAuthenticatedUserEmail: jasmine.createSpy(
        'removeAuthenticatedUserEmail'
      ),
    };
    component.data.booklet = mockBooklet;
    component.removeAuthenticatedUserEmail('test@example.com');
    expect(mockBooklet.removeAuthenticatedUserEmail).toHaveBeenCalledWith(
      'test@example.com'
    );
  });
});
